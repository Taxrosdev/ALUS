use alus::{commit::Commit, repo::Repo};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let commit = Commit::get(&*repo.object_cas, &commit_hash).await?;
    commit.deploy_usr(&repo, usr_path, &sysroot).await?;

    Ok(())
}

fn get_commit_hash() -> io::Result<Vec<u8>> {
    ...
    for part in parts {
        ...
        {
            let hash = hex::decode(val).expect("commit is not hash");
            return Ok(hash);
        }
    }
